...
final class GameParserSpec extends munit.FunSuite:
  test("empty string"):
    val input = ""
    val output = GameParser.parse(input)
    assert(output.isLeft, "an empty string shouldn't parse correctly so it should be Left")

  test("super basic game - draw"):
    val input = "1. e4 d5 1/2-1/2"
    val expectedOutput = Game(
      NonEmptyList.of(
        Turn.Full(
          Move.Pawn(None, false, Square(File.E, Rank.Four), None),
          Move.Pawn(None, false, Square(File.D, Rank.Five), None)
        )
      ),
      Outcome.Draw
    )
    assertEquals(GameParser.parse(input), Right(expectedOutput))
  ...
  test("game with two moves"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      Turn.Full(
        Move.Pawn(None, false, Square(File.E, Rank.Four), None),
        Move.Pawn(None, false, Square(File.D, Rank.Six), None)
      ),
      Turn.Full(
        Move.Pawn(None, false, Square(File.D, Rank.Four), None),
        Move.Standard(Piece.Knight, None, false, Square(File.F, Rank.Six), None)
      )
    ), Outcome.WhiteWins)
    assertEquals(output, Right(expectedOutput))
  ...
  test("game with two moves and a capture"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      Turn.Full(
        Move.Pawn(None, false, Square(File.D, Rank.Four), None),
        Move.Pawn(None, false, Square(File.C, Rank.Five), None)
      ),
      Turn.Full(
        Move.Pawn(Some(Start.FileStart(File.D)), true, Square(File.C, Rank.Five), None),
        Move.Standard(Piece.Knight, None, false, Square(File.A, Rank.Six), None)
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))
  ...
  test("game with file Start"):
    val input = "1. f4 e5 2. d4 c5 3. dxe5 *"
    val output = GameParser.parse(input)
    val expectedOutput = Game(NonEmptyList.of(
      Turn.Full(
        Move.Pawn(None, false, Square(File.F, Rank.Four), None),
        Move.Pawn(None, false, Square(File.E, Rank.Five), None)
      ),
      Turn.Full(
        Move.Pawn(None, false, Square(File.D, Rank.Four), None),
        Move.Pawn(None, false, Square(File.C, Rank.Five), None),
      ...
      Turn.Partial(
        Move.Pawn(Some(Start.FileStart(File.D)), true, Square(File.E, Rank.Five), None)
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))

  test("game with file Start and check but no capture"):
    val input = "1. c4 d5 2. Rfe8+ *" // Note: This is not a valid chess game, but is valid syntax
    val output = GameParser.parse(input)
    val expectedOutput = Game(NonEmptyList.of(
      Turn.Full(
        Move.Pawn(None, false, Square(File.C, Rank.Four), None),
        Move.Pawn(None, false, Square(File.D, Rank.Five), None)
      ),
      Turn.Partial(
        Move.Standard(Piece.Rook, Some(Start.FileStart(File.F)), false, Square(File.E, Rank.Eight), Some(CheckStatus.Check))
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))

  test("game with rank Start"):
    val input = "1. a4 d6 2. R3xd4 *" // Note: This is not a valid chess game, but is valid syntax
    val output = GameParser.parse(input)
    val expectedOutput = Game(NonEmptyList.of(
      Turn.Full(
        Move.Pawn(None, false, Square(File.A, Rank.Four), None),
        Move.Pawn(None, false, Square(File.D, Rank.Six), None)
      ),
      Turn.Partial(
        Move.Standard(Piece.Rook, Some(Start.RankStart(Rank.Three)), true, Square(File.D, Rank.Four), None)
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))

  test("game with rank and file Start"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      ...
      Turn.Partial(
        Move.Standard(
          Piece.Queen,
          Some(Start.SquareStart(Square(File.H, Rank.Eight))),
          true,
  ...
  test("game with pawn promotion"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      ...
      Turn.Partial(
        Move.Promotion(Move.Pawn(None, false, Square(File.B, Rank.Eight), None), Piece.Queen)
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))
  ...
  test("game with pawn promotion and check"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      ...
      Turn.Partial(
        Move.Promotion(Move.Pawn(None, false, Square(File.B, Rank.Eight), Some(CheckStatus.Check)), Piece.Queen)
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))
  ...
  test("game with pawn promotion and capture"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      ...
      Turn.Partial(
        Move.Promotion(Move.Pawn(Some(Start.FileStart(File.C)), true, Square(File.B, Rank.Eight), None), Piece.Queen)
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))
  ...
  test("game with check"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      ...
      Turn.Partial(
        Move.Standard(Piece.Queen, None, false, Square(File.A, Rank.Four), Some(CheckStatus.Check))
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))
  ...
  test("game with castle king side"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      ...
      Turn.Partial(
        Move.Castle(CastleSide.KingSide)
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))
  ...
  test("game with castle queen side"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      ...
      Turn.Partial(
        Move.Castle(CastleSide.QueenSide)
      )
    ), Outcome.Unknown)
    assertEquals(output, Right(expectedOutput))
  ...
  test("game with checkmate"):
    ...
    val expectedOutput = Game(NonEmptyList.of(
      Turn.Full(
        Move.Pawn(None, false, Square(File.F, Rank.Three), None),
        Move.Pawn(None, false, Square(File.E, Rank.Five), None)
      ),
      Turn.Full(
        Move.Pawn(None, false, Square(File.G, Rank.Four), None),
        Move.Standard(Piece.Queen, None, false, Square(File.H, Rank.Four), Some(CheckStatus.Checkmate))
      )
    ), Outcome.BlackWins)
    assertEquals(output, Right(expectedOutput))
  ...
  test("full game - Mamedyarov v Carlsen 2019"):
    val game =
      """|1. d4 d5 2. c4 c6 3. Nc3 Nf6 4. e3 e6 5. Nf3 Nbd7 6. Qc2 Bd6 7. Bd3 dxc4 8. Bxc4
